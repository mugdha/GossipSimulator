...
object project2_bonus {
  ...
  def main(args: Array[String]) {
    ...
    if (args.length > 2) {
      ...
      system.scheduler.schedule(5 seconds, 5 seconds)(killOne)
  ...
  def killOne = {
    ...
    while (rndInt > numNod.toInt || rndInt == 0) {
      rndInt = (Random.nextInt % numNod.toInt).abs
    }
    var targetChild = system.actorFor("akka://GossipSimulator/user/" + rndInt)
    println("Killing Node :: " + rndInt)
    targetChild ! PoisonPill
  }
}
